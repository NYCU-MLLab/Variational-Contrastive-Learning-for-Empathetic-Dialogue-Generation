...
def bleu(hyps_file_path: str, refs_file_path: str, ignore_indices=None, output_path=None) -> tuple:
    ...
    for hyp, ref in zip(hyps, refs):
        ...
        try:
            score = bleu_score.sentence_bleu(
                [ref], hyp, smoothing_function=SmoothingFunction().method7, weights=[0.25, 0.25, 0.25, 0.25])
        except Exception as e:
            print(e)
            score = 0
        bleu_4.append(score)
    bleu_1, bleu_2, bleu_4 = np.average(bleu_1), np.average(bleu_2), np.average(bleu_4)
    output_content = 'BLEU-1/2/4: {}/{}/{}\n'.format(round(bleu_1, 4), round(bleu_2, 4, ), round(bleu_4, 4))
...
def evaluate(task_name: str, hyps_file_path: str, refs_file_path: str, output_path: str, knowledge_file_path=None):
    ...
    assert task_name in ['dailydialog', 'personachat', 'dstc7avsd','empatheticdialog','empatheticdialog_annotated'], \
    ...
    # Empathetic_new    
    elif task_name == 'empatheticdialog':
        bleu(hyps_file_path, refs_file_path, output_path=output_path)
        distinct(hyps_file_path, output_path=output_path)
    elif task_name == 'empatheticdialog_annotated':
        bleu(hyps_file_path, refs_file_path, output_path=output_path)
        distinct(hyps_file_path, output_path=output_path)
    else:
