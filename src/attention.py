...
class NgramMultiheadAttention(nn.Module):
    ...
    def forward(self, query,
        ...
        if not incremental_state and latent_context is not None:
            ...
            # print('latent context is used as key and value')
        ...
        if not incremental_state and latent_context is not None:
            ...
            # Empathetic new, since context extend from one to num_latent context, we use context_k.shape[1] to replace 1
            attn_weights_main[:, :, context_k.shape[1]:] = attn_weights_main[:, :, context_k.shape[1]:] + main_relative_logits
        else:
            main_relative_logits = self.main_stream_relative_logits(
        ...
        if self_attn_mask is not None:
            if not incremental_state and latent_context is not None:
                ...
                # Empathetic new, since context extend from one to num_latent context, we use context_k.shape[1] to replace 1
                attn_weights_main[:, :, context_k.shape[1]:] = attn_weights_main[:, :, context_k.shape[1]:] + self_attn_mask
            else:
                self_attn_mask = self_attn_mask.unsqueeze(0)
        ...
        # Empathetic new, extend condition for ngram=0
        if self.ngram !=0:
            # [ngram, B*head, T, c]
            ...
            if not incremental_state and latent_context is not None:
                # Empathetic new, since context extend from one to num_latent context, we use context_k.shape[1] to replace 1
                k_ngram = torch.cat([torch.cat([k_main[:, context_k.shape[1]:, ], k_p], 1).unsqueeze(0) for k_p in k_predict_list], 0)
            else:
            ...
            if not incremental_state and latent_context is not None:
                # Empathetic new, since context extend from one to num_latent context, we use context_k.shape[1] to replace 1
                v_ngram = torch.cat([torch.cat([v_main[:, context_k.shape[1]:, :], v_p], 1).unsqueeze(0) for v_p in v_predict_list], 0)
            else:
        ...
        if self.ngram !=0:
            ...
            attn = attn_main.view(-1, bsz, embed_dim)
        return attn, None
