...
class NgramTransformerProphetModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--freeze-model', default=False, type=bool, help='freeze encoder weight or not')
        parser.add_argument('--use-adapter', default=False, type=bool, help='use encoder adapter or not')  
        parser.add_argument('--use-decoder-adapter', default=False, type=bool, help='use decoder adapter or not')  
        parser.add_argument('--unfreeze-posterior', default=False, type=bool, help='use adapter or not')  
        parser.add_argument('--freeze-decoder', default=False, type=bool, help='freeze encoder weight or not')
        parser.add_argument('--map-mean-to-latent', default=False, type=bool, help='use mean of tokens instead of cls to transform to latent')
        parser.add_argument('--another-posterior', default=False, type=bool, help='use another posterior to fine-tune model')
        parser.add_argument('--freeze-norm', default=False, type=bool, help='freeze norm weight or not')
        parser.add_argument('--attn-pre', default=False, type=bool, help='decoder attends pre-trained output. If false, attends on fine-tuned output')
    ...
    def build_model(cls, args, task):
        ...
        if args.load_from_pretrained_model is not None:
            ...
            # Empathetic new
            if args.another_posterior:
                # common posterior initialized as pre-trained weights, and do not update
                model.encoder.vae_fc1_com.weight.copy_(state_dict['vae_fc1.weight'])
                model.encoder.vae_fc1_com.bias.copy_(state_dict['vae_fc1.bias'])
                model.encoder.vae_fc2_com.weight.copy_(state_dict['vae_fc2.weight'])
                model.encoder.vae_fc2_com.bias.copy_(state_dict['vae_fc2.bias'])
            
            if args.freeze_model:
                for (name,param) in model.encoder.named_parameters():
                    if 'attn_layer_norm' in name or 'final_layer_norm' in name:
                        if args.freeze_norm:
                            param.requires_grad = False
                        else:
                            print(name +' has gradient')
                            continue
                    if 'adapter' in name or 'vae_fc3' in name or 'class' in name or 'cl_proj' in name:
                        print(name +' has gradient')
                        continue
                    else:
                        if args.unfreeze_posterior or args.another_posterior:
                            if 'vae_fc' in name:
                                if 'com' in name:
                                    # common posterior do not update
                                    param.requires_grad = False
                                else:
                                    print(name +' has gradient')
                                    continue
            ...
            if args.freeze_decoder:
                for (name,param) in model.decoder.named_parameters():
                    # latent transformation should be updated
                    if 'vae' in name:
                        print(name +' has gradient')
                        continue
                    else:
                        param.requires_grad = False
            elif args.use_decoder_adapter:
                for (name,param) in model.decoder.named_parameters():
                    if 'adapter' in name or 'vae' in name  or 'attn_layer_norm' in name or 'final_layer_norm' in name :
                        print(name +' has gradient')
                        continue
                    else:
                        param.requires_grad = False
...
class Adapter(nn.Module):
    def __init__(
            self, embed_dim,down_dim
    ):
        super().__init__()
        self.embed_dim = embed_dim
        self.down_dim = down_dim
        self.adapter = nn.Sequential(  nn.Linear(self.embed_dim, self.down_dim ),
                                       nn.ReLU(),
                                       nn.Linear(self.down_dim , self.embed_dim) 
                                    )
        # self.apply(self._init_weights)
     
    # adapter weights should be initialized near zeros
    def init_weights(self):
        for i_module in self.adapter:
            if isinstance(i_module, nn.Linear):
                i_module.weight.data.normal_(mean=0.0, std = 1e-2)
                # i_module.weight.data.zero_()
                print(str(i_module) + 'init_weighta=a access')
                if i_module.bias is not None:
                    i_module.bias.data.normal_(mean=0.0, std = 1e-2)
                    # i_module.bias.data.zero_()
...
class TransformerEncoderLayer(nn.Module):
    def __init__(
            ...
            embed_relative_positions_token=None, embed_relative_positions_sentence=None,use_adapter=False
    ):
        super().__init__()
        ...
        if use_adapter:
            self.bottle_dim = 64
            self.adapter_1 = Adapter(self.embed_dim, self.bottle_dim)
            self.adapter_2 = Adapter(self.embed_dim, self.bottle_dim)
        

    def forward(self, x, encoder_padding_mask=None, relative_position_token=None, relative_position_sentence=None, use_adapter = False, use_dropout = True):
        """
        Args:
        ...
        if use_adapter:
            x = self.adapter_1(x)
            # print('adapter output is ' + str(x.data))
        # print('residual of  output is ' + str((x - temp_x).data))
        x = residual + x
        # print('difference in x is '+ str(torch.mean(residual[0,:]-x[0,:]).data))
        x = self.self_attn_layer_norm(x)
        # position-wise feed-forward sublayer: x = layer_norm(x + dropout(fc2(dropout(relu(fc1(x))))))
        residual = x
        x = self.activation_fn(self.fc1(x))
        
        # set use dropout flag to avoid change negative samples in common style (Empathetic new)
        if use_dropout:
        ...
        if use_adapter:
            temp_x = x
            x = self.adapter_2(x)
            # print('use_adapter is True')
            # print('adapter output is ' + str(x.data))
            # print('difference in x is '+ str(torch.sum(temp_x[0,:]-x[0,:]).data))
        x = residual + x
...
class TransformerEncoder(FairseqEncoder):
    def __init__(self, args, dictionary, embed_tokens, bow_embed_tokens_enc=None,
        ...
        # with class labels (Empathetic new)
        ...
        self.unfreeze_posterior = args.unfreeze_posterior
        self.cl_style = args.cl_style
        self.map_mean_to_latent = args.map_mean_to_latent
        self.another_posterior = args.another_posterior
        self.attn_pre = args.attn_pre
        ...
        self.layers.extend([
            TransformerEncoderLayer(
                ...
                use_adapter = args.use_adapter #Empatheic new
            )
            for _ in range(args.encoder_layers)
        ])

        self.emb_layer_norm = LayerNorm(self.embed_dim)
        #self.layer_norm = LayerNorm(self.embed_dim) # Empathetic new 將其改為上面名字，以利讀取pre-trained model weight
        ...
        if self.generate_latent_variable:
            ...
            # Empathetic new
            if self.another_posterior:
                self.vae_fc1_com = nn.Linear(self.embed_dim, self.embed_dim)
                self.vae_fc2_com = nn.Linear(self.embed_dim, 2 * args.latent_size)
        ...
        # FNC for emotion and action classifier (Empathetic new)
        if self.emotion_labels: 
            if self.generate_latent_variable and (not self.unfreeze_posterior):
                # self.class_emotion = nn.Sequential(nn.Linear(args.latent_size, args.latent_size*2),
                #                                     nn.LeakyReLU(),
                #                                     nn.Linear(args.latent_size*2,args.emotion_num))
                self.class_emotion = nn.Sequential(nn.Linear(args.latent_size, args.emotion_num))
            else:
                # self.class_emotion = nn.Sequential(nn.Linear(self.embed_dim, 256),
                #                                     nn.LeakyReLU(),
                #                                     nn.Linear(256,args.emotion_num))
                self.class_emotion = nn.Sequential(nn.Linear(self.embed_dim, args.emotion_num))
        if self.action_labels:
            if self.generate_latent_variable and (not self.unfreeze_posterior):
                # self.class_action =  nn.Sequential(nn.Linear(args.latent_size, args.latent_size*2),
                #                                     nn.LeakyReLU(),
                #                                     nn.Linear(args.latent_size*2,args.acttion_num))
                self.class_action = nn.Sequential(nn.Linear(args.latent_size, args.action_num))
            else:
                # self.class_action = nn.Sequential(nn.Linear(self.embed_dim, 256),
                #                                     nn.LeakyReLU(),
                #                                     nn.Linear(256,args.action_num))
                self.class_action = nn.Sequential(nn.Linear(self.embed_dim, args.action_num))
                
        if self.add_cl_loss:
            if self.generate_latent_variable and (not self.unfreeze_posterior): #凍住posterior的情形下，可用latent算contrastive
                self.cl_proj_coherence = nn.Sequential(nn.Linear(args.latent_size,args.cl_proj_dim),
                                                         nn.Tanh()
                                                     )
                self.cl_proj_emotion = nn.Sequential(nn.Linear(args.latent_size,args.cl_proj_dim),
                                                              nn.Tanh()
                                                               )
                self.cl_proj_action = nn.Sequential(nn.Linear(args.latent_size,args.cl_proj_dim),
                                                                nn.Tanh()
                                                             )
            else: #不凍住則只能確保cls token不變，故用self.embed_dim作為input dim

                self.cl_proj_coherence = nn.Sequential(nn.Linear(self.embed_dim ,args.cl_proj_dim),
                                                      nn.Tanh()
                                                      )
                self.cl_proj_emotion = nn.Sequential(nn.Linear(self.embed_dim ,args.cl_proj_dim),
                                                    nn.Tanh()
                                                   )
                self.cl_proj_action = nn.Sequential(nn.Linear(self.embed_dim ,args.cl_proj_dim),
                                                    nn.Tanh()
                                                    )
        if self.cl_style:
            if self.generate_latent_variable and (not self.unfreeze_posterior):
                self.cl_proj_style = nn.Sequential(nn.Linear(args.latent_size,args.cl_proj_dim),
                                                    nn.Tanh()
                                                    )
            else:
                self.cl_proj_style = nn.Sequential(nn.Linear(self.embed_dim,args.cl_proj_dim),
                                                    nn.Tanh()
                                                    )
        ...
        if args.use_adapter:
            for i_layer in self.layers:
                print('i_layer is ' + str(i_layer.adapter_1))
                i_layer.adapter_1.init_weights()
                i_layer.adapter_2.init_weights()
    ...
    def forward_embedding(self, src_tokens, sentence_positions=None, role_positions=None):
        ...
        # x = self.layer_norm(x)  # Empathetic new  將其改為上面名字，以利讀取pre-trained model weight
        x = F.dropout(x, p=self.dropout, training=self.training)
    ...
    def forward(
            ...
            masked_tokens=None, use_adapter = False, **kwargs, 
    ):
        """
        Args:
            ...
            if not self.training or (dropout_probability > self.encoder_layer_drop):
                ...
                if self.add_cl_loss or self.cl_style:
                    if self.embed_rp_sentence is None and self.embed_rp_token is None:
                        with torch.no_grad():
                            # print('here is x_com')
                            x_com= layer(x_com, encoder_padding_mask=encoder_padding_mask,use_adapter = False, use_dropout = False)
                    else:
                        with torch.no_grad():
                            # print('here is x_com')
                            x_com = layer(
                                        x_com, encoder_padding_mask=encoder_padding_mask,
                                        relative_position_token=relative_position_token,
                                        relative_position_sentence=relative_position_sentence,use_adapter = False, use_dropout = False)

                # relative position embedding (PE) here
                if self.embed_rp_sentence is None and self.embed_rp_token is None:
                    # print('here is x')
                    x = layer(x, encoder_padding_mask=encoder_padding_mask,use_adapter = self.use_adapter, use_dropout = True) # Empathetic new , plus use_adapter
                    if self.cl_style:            
                        x_style = layer(x_style, encoder_padding_mask=encoder_padding_mask,use_adapter = self.use_adapter, use_dropout = True)
                        # print('different in x and x_style is ' + str(torch.sum(x-x_style).data))
                else:
                    # print('here is x')
                    x = layer(
                        x, encoder_padding_mask=encoder_padding_mask,
                        relative_position_token=relative_position_token,
                        relative_position_sentence=relative_position_sentence,use_adapter = self.use_adapter, use_dropout = True)
                    if self.cl_style: 
                        x_style = layer(
                                    x_style, encoder_padding_mask=encoder_padding_mask,
                                    relative_position_token=relative_position_token,
                                    relative_position_sentence=relative_position_sentence,use_adapter = self.use_adapter, use_dropout = True)    
                        # print('different in x and x_style is ' + str(torch.sum(x-x_style).data))           
        # print('adapter config is ' + str(self.use_adapter))
        # print('diffrent in x is ' +  str(torch.sum(x[0,:]-x_com[0,:])))
        # for i in self.layers:
        #     for i_para in i.adapter_1.parameters():
        #         print('i_layer adapaters grad is ' + str(i_para.requires_grad))
        ...
        embed_emotion, embed_action = None, None
        embed_cl_emotion, embed_cl_emotion_com , embed_cl_action , embed_cl_action_com , embed_cl , embed_cl_com = None , None, None, None, None, None
        embed_cl_style_anchor, embed_cl_style_pos, embed_cl_style_com = None, None, None
        row_embed, row_embed_com, row_embed_style = None, None, None
        
        
        
        
        if self.with_cls_bow_logits:
            if self.map_mean_to_latent:
                cls_feature = self.map_mean_to_feature(x)
            else:
        ...
        if self.generate_latent_variable:
            ...
            if self.add_cl_loss or self.cl_style:
            # compute embedding which used to projection in contrastive learning
                if self.unfreeze_posterior:
                    row_embed = cls_feature
                    with torch.no_grad():
                        row_embed_com = self.map_cls_to_feature(x_com)
                    if self.cl_style: 
                        row_embed_style = self.map_cls_to_feature(x_style)
                else:
                    row_embed = z
                    with torch.no_grad():
                        if self.another_posterior:
                            # use another posterior network, so common feature from original common posterior
                            cls_com_feature = self.map_cls_to_feature_com(x_com)
                            mu_com, log_var_com = self.sample_com(cls_com_feature) 
                            row_embed_com = self.connect(mu_com, log_var_com, self.deterministic)
                        else:
                            cls_com_feature = self.map_cls_to_feature(x_com)
                            mu_com, log_var_com = self.sample(cls_com_feature)
                            row_embed_com = self.connect(mu_com, log_var_com, self.deterministic)
                    if self.cl_style: 
                        cls_style_feature = self.map_cls_to_feature(x_style)
                        mu_style, log_var_style = self.sample(cls_style_feature)
                        row_embed_style = self.connect(mu_style, log_var_style, self.deterministic)
        ...
        # for non vae mode, Empathetic new
        else:
            row_embed = x[0]
            if self.add_cl_loss or self.cl_style:
                row_embed_com = x_com[0]
                if self.cl_style:
                    row_embed_style = x_style[0]
                
        # calculate class crossentropy (Empathetic new)  
        if self.emotion_labels:
            embed_emotion = self.class_emotion(row_embed)
        if self.action_labels:
            embed_action = self.class_action(row_embed)
                
        if self.cl_action:
            embed_cl_action = self.cl_proj_action(row_embed)    
            embed_cl_action_com = self.cl_proj_action(row_embed_com)
        if self.cl_emotion:
            embed_cl_emotion = self.cl_proj_emotion(row_embed)    
            embed_cl_emotion_com = self.cl_proj_emotion(row_embed_com)
        if self.cl_coherence:
            embed_cl= self.cl_proj_coherence(row_embed)    
            embed_cl_com = self.cl_proj_coherence(row_embed_com)  
        if self.cl_style:
            embed_cl_style_anchor = self.cl_proj_style(row_embed)
            embed_cl_style_pos = self.cl_proj_style(row_embed_style) 
            embed_cl_style_com = self.cl_proj_style(row_embed_com)   
        ...
        if self.attn_pre:
            enc_out = x_com
            print("attend on pre-trained model")
        else:
            enc_out = x

        return {
            'encoder_out': enc_out,                                   # `seq_len, batch_size, embedding_dim`
            ...
            'emotion_logits':embed_emotion,                      # `batch_size, emotion_dim`  #(Empathetic new)
            'action_logits':embed_action ,                        # `batch_size, action_dim`   #(Empathetic new)
            'embed_cl':embed_cl,
            'embed_cl_com':embed_cl_com,
            'embed_cl_action':embed_cl_action,
            'embed_cl_action_com':embed_cl_action_com,
            'embed_cl_emotion':embed_cl_emotion,
            'embed_cl_emotion_com':embed_cl_emotion_com,
            'embed_cl_style_anchor':embed_cl_style_anchor,
            'embed_cl_style_pos':embed_cl_style_pos,
            'embed_cl_style_com':embed_cl_style_com,
            'row_embed':row_embed,
            'row_embed_com':row_embed_com,
            'row_embed_style':row_embed_style,
        }
    ...
    def sample_com(self, feature):
        # this function return mean and log variance of latent variable
        # convert [CLS] feature to mean and log variance
        mu, log_var = self.vae_fc2_com(feature).chunk(2, -1)
        return mu, log_var
    ...
    def map_cls_to_feature_com(self, x):
        """
        Args:
            x: input hidden tensor of shape `seq_len, batch, embedding_dim`
        """
        ...
        hidden = x[0]
        # feature of hidden of [CLS] token, as well as feature of `posterior distribution`
        feature = torch.tanh(self.vae_fc1_com(hidden))
        return feature
    
    def map_mean_to_feature_com(self, x):
        """
        Args:
            x: input hidden tensor of shape `seq_len, batch, embedding_dim`
        """
        # map mean of hiddens to feature to generate mean and log variance of latent variable
        hidden = torch.mean(x,dim=0)
        feature = torch.tanh(self.vae_fc1_com(hidden))
        return feature
    ...
    #Empathetic new
    def compute_cosine_similarity_matrix(self,matrix_a,matrix_b,temp,dim=1):
        nor_matrix_a = matrix_a / matrix_a.norm(dim,p=2)[:,None]
        nor_matrix_b = matrix_b / matrix_b.norm(dim,p=2)[:,None]
        return torch.mm(nor_matrix_a,nor_matrix_b)/temp
    ...
    def reorder_encoder_out(self, encoder_out, new_order):          # -> inference
        ...
        if encoder_out['emotion_logits'] is not None:
            encoder_out['emotion_logits'] = encoder_out['emotion_logits'].index_select(0, new_order) #(Empathetic new)
        if encoder_out['action_logits'] is not None:
            encoder_out['action_logits'] = encoder_out['action_logits'].index_select(0, new_order)  #(Empathetic new)
...
class NgramTransformerDecoderLayer(nn.Module):
    def __init__(
            ...
            dropout, attention_dropout, activation_dropout, activation_fn, with_encoder_decoder_attn,use_decoder_adapter = False
    ):
        super().__init__()
        ...
        # Empathetic new, Try to prevent overfitting by decoder
    ...
    def forward(
            ...
            latent_context=None, use_decoder_adapter = False
    ):
        # multi-head self-attention sublayer: x = layer_norm(x + dropout(self_attn(x)))
        residual = x
...
class NgramTransformerDecoder(FairseqIncrementalDecoder):
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if self.ngram == 2 :
            ...
            self.ngram_input_embed_new = Embedding(self.ngram, self.embed_dim, None)
        else:
            self.ngram_input_embed_none = nn.Embedding(1, self.embed_dim, padding_idx  = 0)
            self.ngram_input_embed_none.weight.data.zero_()
            

        # Empathetic new , try to prevent overfitting in decoder
        ...
        self.layers.extend([
            NgramTransformerDecoderLayer(
                ...
                use_decoder_adapter = self.use_decoder_adapter 
            )
        ...
        # Empathetic new, to extend latent context from one to self.num_latent_context
        self.num_latent_context = 10
        
        if self.use_latent_variable:
            # Empathetic new , extend latent context from one to self.num_latent_context when w/o encoder-decoder-attn
            if  self.with_encoder_decoder_attn:
                self.vae_transform = nn.Linear(
                in_features=args.latent_size, out_features=2 * self.embed_dim)
                self.vae_transform_input = None
            else:
                
                self.vae_transform_input = None
                self.vae_transform_extend = nn.Linear(
                in_features=args.latent_size, out_features=2*self.num_latent_context * self.embed_dim) # extend num of key and value from 1 to 10
        else:
            # original
            # self.vae_transform = None
            if  self.with_encoder_decoder_attn:
                self.vae_transform = nn.Linear(
                    in_features= self.embed_dim, out_features=2 * self.embed_dim)
            else:
                self.vae_transform_extend = nn.Linear(
                    in_features= self.embed_dim, out_features=2 *self.num_latent_context * self.embed_dim)
    ...
    def forward(
        ...
        #Empathetic new, to extend condition on ngram=0
        if self.ngram!=0:
    ...
    def transform_latent_context(self, latent_context):
        ...
        # Empathetic new , extend latent context from one pair to ten pairs
        if not self.with_encoder_decoder_attn:
            latent_context = self.vae_transform_extend(latent_context)
            latent_context = latent_context.view(-1, 2*self.num_latent_context , self.head_dim)
            latent_context = torch.split(latent_context, self.num_latent_context, dim=1)
        else:
            latent_context = self.vae_transform(latent_context)
    ...
    def extract_features(
        ...
        x = x.transpose(0, 1)
        # Empathetic new,  decoder input add latent to enhance relation with latent
        if self.use_latent_variable and (self.vae_transform_input is not None) :
            # print('x shape is ' + str(x.shape))
            # print('latent shape is ' + str(encoder_out['z'].shape))
            x += self.vae_transform_input(encoder_out['z'])
            # print('vae_transform_input is achieved')
        ...
        # Empathetic new, try to extend condition when ngram=0
        if self.ngram ==2:
            if self.embed_scale is not None :
            ...
            if self.embed_scale is not None :
                ngram_input_embed = self.embed_scale * self.ngram_input_embed_new.weight
            else:
                ngram_input_embed = self.ngram_input_embed_new.weight
        ...
        if not self.use_latent_variable:
            latent_context = self.transform_latent_context(encoder_out['encoder_out'][0])
        else:
            latent_context = self.transform_latent_context(encoder_out['z'])
            # print('latent context transform is achieve')
...
def base_architecture(args):
    ...
    # Empathetic new
    # args.emotion_labels = getattr(args, 'emotion_labels', True) 
    args.emotion_embed_dim = getattr(args, 'emotion_embed_dim', 512) 
    args.emotion_num = getattr(args, 'emotion_num', 32) 
    # args.action_labels = getattr(args, 'action_labels', True) 
    args.action_embed_dim = getattr(args, 'action_embed_dim', 512) 
    args.action_num = getattr(args, 'action_num', 9) 
    args.cl_proj_dim = getattr(args, 'cl_proj_dim', 256) 
...
@register_model_architecture('ngram_transformer_prophet', 'ngram_transformer_prophet_seq2seq_latent')
def transformer_seq2seq_latent(args):
    # this parameter is the key difference between seq2seq-based and vae-based models
    args.generate_latent_variable = getattr(args, 'generate_latent_variable', False)
...
@register_model_architecture('ngram_transformer_prophet', 'ngram_transformer_prophet_vae_standard_latent')
def transformer_vae_standard_latent(args):
    args.generate_latent_variable = getattr(args, 'generate_latent_variable', True)
    args.with_encoder_decoder_attn = getattr(args, 'with_encoder_decoder_attn', False)
    args.use_latent_variable = getattr(args, 'use_latent_variable', True)
...
@register_model_architecture('ngram_transformer_prophet', 'ngram_transformer_prophet_vae_large_latent')
def transformer_vae_large_latent(args):
    args.generate_latent_variable = getattr(args, 'generate_latent_variable', True)
    args.with_encoder_decoder_attn = getattr(args, 'with_encoder_decoder_attn', False)
    args.use_latent_variable = getattr(args, 'use_latent_variable', True)
...
@register_model_architecture('ngram_transformer_prophet', 'ngram_transformer_prophet_seq2seq_latent_no_ngram')
def transformer_seq2seq_latent_no_ngram(args):
    # this parameter is the key difference between seq2seq-based and vae-based models
    args.generate_latent_variable = getattr(args, 'generate_latent_variable', False)
    args.with_encoder_decoder_attn = getattr(args, 'with_encoder_decoder_attn', False)
    args.use_latent_variable = getattr(args, 'use_latent_variable', False)
    ...
    setattr(args, 'ngram', 0)
    
    
@register_model_architecture('ngram_transformer_prophet', 'ngram_transformer_prophet_seq2seq_no_ngram')
def transformer_seq2seq_no_ngram(args):
    # this parameter is the key difference between seq2seq-based and vae-based models
    args.generate_latent_variable = getattr(args, 'generate_latent_variable', False)
    args.with_encoder_decoder_attn = getattr(args, 'with_encoder_decoder_attn', True)
    args.use_latent_variable = getattr(args, 'use_latent_variable', False)
...
@register_model_architecture('ngram_transformer_prophet', 'ngram_transformer_prophet_vae_large_no_ngram')
def transformer_vae_large_no_ngram(args):
    # this parameter is the key difference between seq2seq-based and vae-based models
    args.generate_latent_variable = getattr(args, 'generate_latent_variable', True)
    args.with_encoder_decoder_attn = getattr(args, 'with_encoder_decoder_attn', True)
    args.use_latent_variable = getattr(args, 'use_latent_variable', True)
...
# ----------------------------------------------------------------------------------------------------------------------   
@register_model_architecture('ngram_transformer_prophet', 'ngram_transformer_prophet_seq2seq_mean_latent_no_ngram')
def transformer_seq2seq_mean_latent_no_ngram(args):
    # this parameter is the key difference between seq2seq-based and vae-based models
    args.generate_latent_variable = getattr(args, 'generate_latent_variable', False)
    args.with_encoder_decoder_attn = getattr(args, 'with_encoder_decoder_attn', True)
    args.use_latent_variable = getattr(args, 'use_latent_variable', False)
    ...
    args.extend_latent_with_cls = getattr(args, 'extend_latent_with_cls', False)
    setattr(args, 'ngram', 0)   
    args.map_mean_to_latent = getattr(args, 'map_mean_to_latent', True)   


@register_model_architecture('ngram_transformer_prophet', 'ngram_transformer_prophet_vae_large_mean_latent_no_ngram')
def transformer_prophet_vae_large_mean_latent_no_ngram(args):
    # this parameter is the key difference between seq2seq-based and vae-based models
    args.generate_latent_variable = getattr(args, 'generate_latent_variable', True)
    args.with_encoder_decoder_attn = getattr(args, 'with_encoder_decoder_attn', True)
    args.use_latent_variable = getattr(args, 'use_latent_variable', True)
    ...
    args.extend_latent_with_cls = getattr(args, 'extend_latent_with_cls', False)
    setattr(args, 'ngram', 0)
    setattr(args, 'map_mean_to_latent', False)  
...
@register_model_architecture('ngram_transformer_prophet', 'ngram_transformer_prophet_vae_large_no_ngram_another_posterior')
def transformer_prophet_vae_large_no_ngram_another_posterior(args):
    # this parameter is the key difference between seq2seq-based and vae-based models
    args.generate_latent_variable = getattr(args, 'generate_latent_variable', True)
    args.with_encoder_decoder_attn = getattr(args, 'with_encoder_decoder_attn', True)
    args.use_latent_variable = getattr(args, 'use_latent_variable', True)
    args.another_posterior = getattr(args, 'another_posterior', True)
...
# ----------------------------------------------------------------------------------------------------------------------   
@register_model_architecture('ngram_transformer_prophet', 'ngram_transformer_prophet_vae_large_no_ngram_attn_pre')
def transformer_prophet_vae_large_no_ngram_attn_pre(args):
    # this parameter is the key difference between seq2seq-based and vae-based models
    args.generate_latent_variable = getattr(args, 'generate_latent_variable', True)
    args.with_encoder_decoder_attn = getattr(args, 'with_encoder_decoder_attn', True)
    args.use_latent_variable = getattr(args, 'use_latent_variable', True)
    args.another_posterior = getattr(args, 'another_posterior', True)
    args.attn_pre = getattr(args, 'attn_pre', True)
