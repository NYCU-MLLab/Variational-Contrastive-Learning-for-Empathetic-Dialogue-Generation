...
def collate(
    ...
    # Empathetic new
    emotion = None
    if samples[0].get('emotion', None) is not None:
        # emotion_temp = merge_labels('emotion')
        # emotion = []
        # for num_i in sort_order:
        #     emotion.append(emotion_temp[num_i])
        emotion = torch.LongTensor([s['emotion'] for s in samples])
            
        # 原先的index_select似乎是給tensor用。不確定這裡的emotion、action是否已被轉為tensor。
        # 若不是，則要再找給list用的 index_select。
    action = None
    if samples[0].get('action', None) is not None:
        # action_temp = merge_labels('action')
        # action=[]
        # for num_j in sort_order:
        #     action.append(action_temp[num_j])
        action = torch.LongTensor([s['action'] for s in samples])
...
class LanguagePairDatasetVED(LanguagePairDataset):
    def __init__(
            self, src, src_sizes, src_dict,   # 此處回傳的src,tgt已經是在task中，處理成dataset的形式，而非單純的字串
            ...
            # Empathetic new
            emotion_labels = False, emotion_tgt = None, emotion_dict = None,
            action_labels = False, action_tgt = None, action_dict = None
            ):
        ...
        # whether add labels or not (Empathetic new)
        self.emotion_labels = emotion_labels 
        self.emotion_tgt = emotion_tgt
        self.emotion_dict = emotion_dict
        self.action_labels = action_labels
        self.action_tgt = action_tgt
    ...
    def __getitem__(self, index):
        ...
        # add emotion and action labels in samples (Empathetic new)
        if self.emotion_labels :
            # since first 4 token in dict is <bos>, <pad>, <eos> ,<unk>, appended by Dictionary class
            # we get index and minus 4 to get true label index
            # original label sentence just one token, but it appended with eos at the end.
            # so we just take first token as label.  
            example['emotion'] = self.emotion_tgt[index][0] - 4  
            if self.emotion_tgt[index][0]<4 or self.emotion_tgt[index][0]>35:
                print('emotion labels has error')
                
        if self.action_labels:
            example['action'] = self.action_tgt[index][0] - 4
            if self.action_tgt[index][0]<4 or self.action_tgt[index][0]>12:
                print('action labels has error')
        return example
